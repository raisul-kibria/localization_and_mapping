...
# -*- coding: utf-8 -*-
'''
Implementation of Fast SLAM 1.0 with unknown correspondences.

The difference from known correspondence lies in the data_association part.

See Probabilistic Robotics:
    1. Page 461, Table 13.2 for full algorithm.
...
class FastSLAM1():
    ...
    def landmark_update(self, measurement):
        ...
        for particle in self.particles:
            # Get landmark index
            landmark_idx = self.data_association(particle, measurement)
    ...
    def data_association(self, particle, measurement):
        ...
        # corresponding index, and then initilize it at the measured location
        if not particle.lm_ob[self.landmark_indexes[measurement[1]]]:
            return self.landmark_indexes[measurement[1]]

        # Compute likelihood of correspondence for all observed landmarks
        max_likelihood = 0.0
        max_idx = 0
